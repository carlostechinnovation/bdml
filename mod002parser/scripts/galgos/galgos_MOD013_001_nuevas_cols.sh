...
CREATE TABLE datos_desa.tb_ce_x1a AS 
SELECT 1 AS distancia_tipo, MIN(vel_going_cortas_max) AS valor_min, MAX(vel_going_cortas_max) AS valor_max 
FROM datos_desa.tb_galgos_agregados_LIM 
UNION 
SELECT 2 AS distancia_tipo, MIN(vel_going_longmedias_max) AS valor_min, MAX(vel_going_longmedias_max) AS valor_max 
FROM datos_desa.tb_galgos_agregados_LIM 
UNION 
SELECT 3 AS distancia_tipo, MIN(vel_going_largas_max) AS valor_min, MAX(vel_going_largas_max) AS valor_max 
FROM datos_desa.tb_galgos_agregados_LIM;
...
SELECT galgo_nombre,
(vel_going_cortas_mediana - (select valor_min FROM datos_desa.tb_ce_x1a WHERE distancia_tipo=1) ) AS vgcortas_med_min,
(vel_going_cortas_mediana - (select valor_max FROM datos_desa.tb_ce_x1a WHERE distancia_tipo=1) ) AS vgcortas_med_max,
(vel_going_longmedias_mediana - (select valor_min FROM datos_desa.tb_ce_x1a WHERE distancia_tipo=2) ) AS vgmedias_med_min,
(vel_going_longmedias_mediana - (select valor_max FROM datos_desa.tb_ce_x1a WHERE distancia_tipo=2) ) AS vgmedias_med_max,
(vel_going_largas_mediana - (select valor_min FROM datos_desa.tb_ce_x1a WHERE distancia_tipo=3) ) AS vglargas_med_min,
(vel_going_largas_mediana - (select valor_max FROM datos_desa.tb_ce_x1a WHERE distancia_tipo=3) ) AS vglargas_med_max
FROM datos_desa.tb_galgos_agregados_LIM;
...
FROM datos_desa.tb_galgos_historico_LIM GH,
...
FROM (select trap,posicion,count(*) as contador FROM datos_desa.tb_galgos_historico_LIM GROUP BY trap,posicion ORDER BY trap ASC, posicion ASC) dentro 
...
SELECT id_carrera, galgo_nombre, sp
FROM datos_desa.tb_galgos_historico_LIM GH;
...
read -d '' CONSULTA_X5 <<- EOF
DROP TABLE IF EXISTS datos_desa.tb_ce_x5;
CREATE TABLE datos_desa.tb_ce_x5 AS SELECT id_carrera, galgo_nombre, clase FROM datos_desa.tb_galgos_historico_LIM GH;
ALTER TABLE datos_desa.tb_ce_x5 ADD INDEX tb_ce_x5_idx(id_carrera, galgo_nombre);
SELECT * FROM datos_desa.tb_ce_x5 LIMIT 5;
SELECT count(*) as num_x5 FROM datos_desa.tb_ce_x5 LIMIT 5;
EOF

echo -e "$CONSULTA_X5" 2>&1 1>>${LOG_013}
mysql --execute="$CONSULTA_X5" >>${LOG_013}
}
...
FROM datos_desa.tb_galgos_historico_LIM GH
LEFT JOIN datos_desa.tb_galgos_historico_LIM GH2 ON (GH.galgo_nombre=GH2.galgo_nombre AND GH.clase=GH2.clase)
;
...
cruce1.experiencia_en_clase AS experiencia_en_clase,
...
X6B.posicion_media_en_clase_por_experiencia AS posicion_media_en_clase_por_experiencia,
...
FROM datos_desa.tb_galgos_posiciones_en_carreras_LIM PO 
LEFT JOIN (select id_carrera, MAX(distancia) AS distancia FROM datos_desa.tb_galgos_historico_LIM GROUP BY id_carrera) GH 
...
(
  ...
  FROM datos_desa.tb_galgos_historico_LIM GH
  LEFT JOIN (SELECT galgo_nombre, MAX(peso_galgo) AS peso_galgo FROM datos_desa.tb_galgos_posiciones_en_carreras_LIM GROUP BY galgo_nombre) PO
...
SELECT track,
STD(going_abs) AS venue_going_std, AVG(going_abs) AS venue_going_avg 
FROM (select track, ABS(going_allowance_segundos) AS going_abs FROM datos_desa.tb_galgos_carreras_LIM) dentro 
...
SELECT entrenador, 
posicion_avg,
-- ((6-posicion_avg)/5) AS entrenador_posicion_norm
posicion_std
FROM datos_desa.tb_ce_x9a
WHERE entrenador IS NOT NULL && entrenador <>'UNKNOWN';
...
-- set @min_vrc_med=(select MIN(vel_real_cortas_mediana) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vrc_med=(select CASE WHEN MIN(vel_real_cortas_mediana)=0 THEN MAX(vel_real_cortas_mediana) ELSE MAX(vel_real_cortas_mediana)-MIN(vel_real_cortas_mediana) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vrc_max=(select MIN(vel_real_cortas_max) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vrc_max=(select CASE WHEN MIN(vel_real_cortas_max)=0 THEN MAX(vel_real_cortas_max) ELSE MAX(vel_real_cortas_max)-MIN(vel_real_cortas_max) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vgc_med=(select MIN(vel_going_cortas_mediana) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vgc_med=(select CASE WHEN MIN(vel_going_cortas_mediana)=0 THEN MAX(vel_going_cortas_mediana) ELSE MAX(vel_going_cortas_mediana)-MIN(vel_going_cortas_mediana) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vgc_max=(select MIN(vel_going_cortas_max) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vgc_max=(select CASE WHEN MIN(vel_going_cortas_max)=0 THEN MAX(vel_going_cortas_max) ELSE MAX(vel_going_cortas_max)-MIN(vel_going_cortas_max) END FROM datos_desa.tb_galgos_agregados_LIM);

-- set @min_vrlm_med=(select MIN(vel_real_longmedias_mediana) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vrlm_med=(select CASE WHEN MIN(vel_real_longmedias_mediana)=0 THEN MAX(vel_real_longmedias_mediana) ELSE MAX(vel_real_longmedias_mediana)-MIN(vel_real_longmedias_mediana) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vrlm_max=(select MIN(vel_real_longmedias_max) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vrlm_max=(select CASE WHEN MIN(vel_real_longmedias_max)=0 THEN MAX(vel_real_longmedias_max) ELSE MAX(vel_real_longmedias_max)-MIN(vel_real_longmedias_max) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vglm_med=(select MIN(vel_going_longmedias_mediana) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vglm_med=(select CASE WHEN MIN(vel_going_longmedias_mediana)=0 THEN MAX(vel_going_longmedias_mediana) ELSE MAX(vel_going_longmedias_mediana)-MIN(vel_going_longmedias_mediana) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vglm_max=(select MIN(vel_going_longmedias_max) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vglm_max=(select CASE WHEN MIN(vel_going_longmedias_max)=0 THEN MAX(vel_going_longmedias_max) ELSE MAX(vel_going_longmedias_max)-MIN(vel_going_longmedias_max) END FROM datos_desa.tb_galgos_agregados_LIM);

-- set @min_vrl_med=(select MIN(vel_real_largas_mediana) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vrl_med=(select CASE WHEN MIN(vel_real_largas_mediana)=0 THEN MAX(vel_real_largas_mediana) ELSE MAX(vel_real_largas_mediana)-MIN(vel_real_largas_mediana) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vrl_max=(select MIN(vel_real_largas_max) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vrl_max=(select CASE WHEN MIN(vel_real_largas_max)=0 THEN MAX(vel_real_largas_max) ELSE MAX(vel_real_largas_max)-MIN(vel_real_largas_max) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vgl_med=(select MIN(vel_going_largas_mediana) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vgl_med=(select CASE WHEN MIN(vel_going_largas_mediana)=0 THEN MAX(vel_going_largas_mediana) ELSE MAX(vel_going_largas_mediana)-MIN(vel_going_largas_mediana) END FROM datos_desa.tb_galgos_agregados_LIM);
-- set @min_vgl_max=(select MIN(vel_going_largas_max) FROM datos_desa.tb_galgos_agregados_LIM);
-- set @diff_vgl_max=(select CASE WHEN MIN(vel_going_largas_max)=0 THEN MAX(vel_going_largas_max) ELSE MAX(vel_going_largas_max)-MIN(vel_going_largas_max) END FROM datos_desa.tb_galgos_agregados_LIM);
...
DROP TABLE IF EXISTS datos_desa.tb_gh_y_remarkspuntos_LIM1;

CREATE TABLE datos_desa.tb_gh_y_remarkspuntos_LIM1 AS
...
ALTER TABLE datos_desa.tb_gh_y_remarkspuntos_LIM1 ADD INDEX tb_gh_y_remarkspuntos_LIM1_idx1(galgo_nombre, id_carrera);
ALTER TABLE datos_desa.tb_gh_y_remarkspuntos_LIM1 ADD INDEX tb_gh_y_remarkspuntos_LIM1_idx2(galgo_nombre, fecha);

SELECT count(*) AS num_ghyrmp FROM datos_desa.tb_gh_y_remarkspuntos_LIM1 LIMIT 5;
SELECT * FROM datos_desa.tb_gh_y_remarkspuntos_LIM1 LIMIT 10;
EOF
...
DROP TABLE IF EXISTS datos_desa.tb_gh_y_remarkspuntos_LIM2;

CREATE TABLE datos_desa.tb_gh_y_remarkspuntos_LIM2 AS
SELECT 
...
FROM datos_desa.tb_gh_y_remarkspuntos_LIM1 A
LEFT OUTER JOIN datos_desa.tb_gh_y_remarkspuntos_LIM1 B ON (A.galgo_nombre=B.galgo_nombre AND A.fecha >= B.fecha  )
ORDER BY a_nombre ASC, a_fechaint DESC
;

ALTER TABLE datos_desa.tb_gh_y_remarkspuntos_LIM2 ADD INDEX tb_gh_y_remarkspuntos_LIM2_idx1(a_nombre, a_fechaint);
ALTER TABLE datos_desa.tb_gh_y_remarkspuntos_LIM2 ADD INDEX tb_gh_y_remarkspuntos_LIM2_idx2(a_nombre, a_idc, a_fechaint);
SELECT count(*) AS num_ghyrmp2 FROM datos_desa.tb_gh_y_remarkspuntos_LIM2 LIMIT 10;
SELECT * FROM datos_desa.tb_gh_y_remarkspuntos_LIM2 LIMIT 10;
EOF
...
DROP TABLE IF EXISTS datos_desa.tb_gh_y_remarkspuntos_LIM3;

CREATE TABLE datos_desa.tb_gh_y_remarkspuntos_LIM3 AS

SELECT 
T_todos.galgo_nombre, T_todos.id_carrera, T_todos.fecha, 

T_todos.remarks_puntos_historico AS remarks_puntos_historico,
...
(T50D.remarks_puntos_historico_50d) AS remarks_puntos_historico_50d

FROM 
  (SELECT a_nombre AS galgo_nombre, a_idc AS id_carrera,  a_fechaint AS fecha, AVG(b_puntos) as remarks_puntos_historico FROM datos_desa.tb_gh_y_remarkspuntos_LIM2 GROUP BY a_nombre, a_idc, a_fechaint ) T_todos
  
  LEFT JOIN 
  (SELECT a_nombre AS galgo_nombre, a_fechaint AS fecha, AVG(b_puntos) as remarks_puntos_historico_10d FROM datos_desa.tb_gh_y_remarkspuntos_LIM2 WHERE diff_fecha<=10 GROUP BY a_nombre, a_fechaint ) T10D
  ON (T_todos.galgo_nombre = T10D.galgo_nombre AND T_todos.fecha = T10D.fecha)

  LEFT JOIN 
  (SELECT a_nombre AS galgo_nombre, a_fechaint AS fecha, AVG(b_puntos) as remarks_puntos_historico_20d FROM datos_desa.tb_gh_y_remarkspuntos_LIM2 WHERE diff_fecha<=20 GROUP BY a_nombre, a_fechaint ) T20D
  ON (T_todos.galgo_nombre = T20D.galgo_nombre AND T_todos.fecha = T20D.fecha)

  LEFT JOIN 
  (SELECT a_nombre AS galgo_nombre, a_fechaint AS fecha, AVG(b_puntos) as remarks_puntos_historico_50d FROM datos_desa.tb_gh_y_remarkspuntos_LIM2 WHERE diff_fecha<=50 GROUP BY a_nombre, a_fechaint ) T50D
  ON (T_todos.galgo_nombre = T50D.galgo_nombre AND T_todos.fecha = T50D.fecha)
;

ALTER TABLE datos_desa.tb_gh_y_remarkspuntos_LIM3 ADD INDEX tb_gh_y_remarkspuntos_LIM3_idx(galgo_nombre, id_carrera);
SELECT count(*) AS num_ghyrmp3 FROM datos_desa.tb_gh_y_remarkspuntos_LIM3 LIMIT 10;
SELECT * FROM datos_desa.tb_gh_y_remarkspuntos_LIM3 LIMIT 10;
EOF
...
# Limpieza
mysql --execute="DROP TABLE IF EXISTS datos_desa.tb_gh_y_remarkspuntos_LIM1;" 2>&1 1>>${LOG_013}
mysql --execute="DROP TABLE IF EXISTS datos_desa.tb_gh_y_remarkspuntos_LIM2;" 2>&1 1>>${LOG_013}
...
SELECT GHRPNM3.* FROM datos_desa.tb_gh_y_remarkspuntos_LIM3 GHRPNM3;
