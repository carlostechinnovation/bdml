...
mysql -tN --execute="DROP TABLE IF EXISTS datos_desa.tb_galgos_posiciones_en_carreras_LIM; CREATE TABLE datos_desa.tb_galgos_posiciones_en_carreras_LIM AS SELECT * FROM datos_desa.tb_galgos_posiciones_en_carreras WHERE galgo_nombre <>'VACANT' AND sp<12;" 2>&1 1>>"${LOG_011}"

echo -e "LIMPIANDO tb_galgos_historico..." >> "${LOG_011}"
mysql -tN --execute="DROP TABLE IF EXISTS datos_desa.tb_galgos_historico_LIM; CREATE TABLE datos_desa.tb_galgos_historico_LIM AS SELECT * FROM datos_desa.tb_galgos_historico WHERE (distancia IS NULL OR (distancia>100 AND distancia<=750)) AND trap<=6 AND (velocidad_con_going IS NULL OR (velocidad_con_going>=15 AND velocidad_con_going<=18.1)) AND clase IS NOT NULL AND galgo_nombre <>'VACANT' AND edad_en_dias>90 AND edad_en_dias<2400 AND sp<12;" 2>&1 1>>"${LOG_011}"
