...
PATH_WARNINGS="/home/carloslinux/git/bdml/mod002parser/scripts/bolsa/warnings_2a_yf"
...
do
    ...
    mysql -e "TRUNCATE TABLE datos_desa.tb_yf01_previa\W;" >>$PATH_WARNINGS

    mysql -e "LOAD DATA LOCAL INFILE '${path_fichero_limpio}' INTO TABLE datos_desa.tb_yf01_previa FIELDS TERMINATED BY '|' LINES TERMINATED BY '\n' IGNORE 0 LINES\W;"  >>$PATH_WARNINGS

    sleep 1s    

    echo -e "Insertando empresa: "${yf_empresa}
    mysql -e "DELETE FROM datos_desa.tb_yf01 WHERE ticker=${yf_empresa} AND date >=${anio}0000 AND date <=${anio}9999\W;" >>$PATH_WARNINGS
...
mysql -e "SELECT COUNT(*) as contador FROM datos_desa.tb_yf01 LIMIT 1\W;" >>$PATH_WARNINGS
