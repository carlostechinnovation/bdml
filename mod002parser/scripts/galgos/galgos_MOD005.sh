...
#########################
cat "/home/carloslinux/Desktop/CODIGOS/workspace_java/bdml/mod002parser/scripts/galgos/galgos_MOD004.out" | grep 'Gana modelo' >>$PATH_INFORME_FINAL
...
echo -e "\nDETALLE:\n"
mysql -u root --password=datos1986 -t --execute="SELECT dia, hora, estadio, galgo_nombre, PREDICCION FROM datos_desa.tb_galgos_data_final A LEFT JOIN datos_desa.tb_galgos_target_final B ON (A.rank=B.rank) WHERE PREDICCION=1 ORDER BY dia DESC, hora DESC, estadio DESC, galgo_nombre DESC;" >>${PATH_INFORME_FINAL}
