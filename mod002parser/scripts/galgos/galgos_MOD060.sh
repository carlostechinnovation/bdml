...
echo -e $(date +"%T")"Modulo 006 - ANALISIS POSTERIOR"
...
echo -e $(date +"%T")"Carrera analizada = $idcarrera_organizada"
echo -e $(date +"%T")"Path donde ponemos los CSVs = $FOLDER_006"


echo -e $(date +"%T")"GAGST - TABLAS de datos originales (tras parsear webs)..."
...
echo -e $(date +"%T")"GAGST - TABLAS de entrenamiento (pre)..."
...
mysql -u root --password=datos1986 -t --execute="SELECT * FROM datos_desa.tb_galgos_002_ordenadoporcriteriocompuesto_gagst_pre WHERE id_carrera=$idcarrera_organizada LIMIT 100\W;" > "${FOLDER_006}gagst_pre_002b.csv"
...
echo -e $(date +"%T")"GAGST - TABLAS de TEST (post)..."
...
mysql -u root --password=datos1986 -t --execute="SELECT * FROM datos_desa.tb_galgos_002_ordenadoporcriteriocompuesto_gagst_post LIMIT 0\W;" > "${FOLDER_006}gagst_post_002b.csv"
...
echo -e $(date +"%T")"Analisis posterior: FIN"
